...
class UserClass {
    ...
    //签到
    async userInfo() {
        let sign = MD5Encrypt(`a=wxapp&c=entry&do=user&dopost=make_sign&from=wxapp&i=2&m=skai_tooln&t=0&userid=${this.xr}&v=1.3undefined`)
        let options = {
            fn: 'userInfo',
            method: 'GET',
            url: `${this.hostname}/app/index.php?i=2&t=0&v=1.3&from=wxapp&c=entry&a=wxapp&do=user&sign=${sign}&m=skai_tooln&dopost=make_sign&userid=${this.xr}`,
        ...
        // console.log(resp)
         if (resp.result == "success") {
            $.log(`${this.idx}: 签到天数: ${resp.sign_total} `)
	...
	// 看视频
    async ksp() {
        let sign = MD5Encrypt(`a=wxapp&c=entry&do=user&dopost=get_some_power_ad_video&from=wxapp&i=2&m=skai_tooln&t=0&userid=${this.xr}&v=1.3undefined`)        
        let options = {
            fn: 'ksp',
            method: 'GET',
            url: `${this.hostname}/app/index.php?i=2&t=0&v=1.3&from=wxapp&c=entry&a=wxapp&do=user&sign=${sign}&m=skai_tooln&dopost=get_some_power_ad_video&userid=${this.xr}`,
            headers: this.qd_headers,
        }
        // console.log(options)
        let resp = await $.request(options)
        // console.log(resp)
         if (resp.result == "success") {
			$.log(`${this.idx}: 获得: ${resp.addpower} 能量值 \n今日已看视频数:${resp.userdata.ad_video_num}\n`)
        } else if (resp.result == "fail") {
            $.log(`\n 状况:${resp.msg}\n`)
            this.ckFlog = false
        }  
